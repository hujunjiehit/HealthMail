...
public class CancleGuanzhuActivity extends BaseActivity implements View.OnClickListener{
    ...
    public void handleMessage(Message msg) {
      switch (msg.what){
        case START_TO_GUAN_ZHU:
          if(isRunning) {
            if (accountIndex < accountList.size()) {
              showTheResult("\n\n第" + (accountIndex + 1) + "个号：" + accountList.get(accountIndex).getPhoneNumber() + "开始取消关注\n");
              ...
              showTheResult("******所有账号取消关注结束**********\n");
              isRunning = false;
              btn_start.setText("取消关注完成");
            }
          } else {
            showTheResult("**用户自己终止取消关注进展**当前已经执行完成"+ accountIndex + "个小号\n");
          }
          break;
        ...
        case GET_GUANZHU_LIST_SUCCESS:
          ...
          if(targetInTheList) {
            showTheResult("*****************已关注目标私教，开始取消关注\n");
            message = this.obtainMessage(DO_THE_ACTION_GUANZHU);
            message.sendToTarget();
          } else {
            showTheResult("*****************未关注目标私教，继续下一个小号\n");
            accountIndex++;
            message = this.obtainMessage(START_TO_GUAN_ZHU);
            message.sendToTarget();
          }
        ...
        case DO_THE_ACTION_GUANZHU:
          showTheResult("------------------------------发送取消关注申请\n");
          postGuanzhuApply();
          break;
        case GUANZHU_SUCESS:
          showTheResult("----------------------------------取消关注成功\n");
          accountIndex++;
          this.sendEmptyMessageDelayed(START_TO_GUAN_ZHU,getDelayTime());
          break;
        case GUANZHU_FAILED:
          showTheResult("----------------------------------取消关注失败，继续尝试\n");
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_cancle_guanzhu);
  ...
  public void onClick(View v) {
    switch (v.getId()){
      case R.id.btn_start:
        if("取消关注完成".equals(btn_start.getText().toString().trim())){
          Toast.makeText(this,"取消关注已完成，如需继续取消关注请重新进入本页面",Toast.LENGTH_LONG).show();
        }else {
          if (isRunning == false) {
            isRunning = true;
            btn_start.setText("停止取消关注");
            startToGuanzhu();
          } else {
            isRunning = false;
            mHandler.removeCallbacksAndMessages(null);
            btn_start.setText("开始取消关注");
          }
        }
