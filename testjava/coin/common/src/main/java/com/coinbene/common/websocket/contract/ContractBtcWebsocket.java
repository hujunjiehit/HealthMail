 ...
 * on 2019-08-04
 */
public class ContractBtcWebsocket extends BaseGloabalWebsocket {

	private static volatile ContractBtcWebsocket mInstance;

	private static final String WS_TRADEPAIR_KEY = "contractBtcWsTradePair";
	...
	private ContractBtcWebsocket() {
		contractName = SpUtil.get(CBRepository.getContext(), SpUtil.CONTRACT_COIN, "BTCUSDT");
		if (TextUtils.isEmpty(contractName) || !ContractInfoController.getInstance().checkContractIsExist(contractName)) {
		...
		mOrderBookSubscriber = new CBWebSocketSubscriber<WsContractOrderBookModel>(Constants.BASE_WEBSOCKET_CONTRACT_BTC, orderBookTopic) {
			@Override
		...
		mTradeDetailSubscriber = new CBWebSocketSubscriber<WsTradeDetailModel>(Constants.BASE_WEBSOCKET_CONTRACT_BTC, tradeDetailTopic) {
			@Override
...
//			throw new StackOverflowError();
	...
	public static ContractBtcWebsocket getInstance() {
		if (mInstance == null) {
			synchronized (ContractBtcWebsocket.class) {
				if (mInstance == null) {
					mInstance = new ContractBtcWebsocket();
				}
			}
		}
		return mInstance;
