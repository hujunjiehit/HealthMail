 ...
 * on 2020-01-15
 */
public class NewContractBtcWebsocket extends BaseWebSocket implements WebsocketOperatiron {

	private static final String WS_TRADEPAIR_KEY = "contractBtcWsTradePair";

	private static volatile NewContractBtcWebsocket mInstance;
	...
	private NewContractBtcWebsocket() {

		tickerData = new WsMarketData();

		symbol = MMKV.defaultMMKV().decodeString(WS_TRADEPAIR_KEY, "BTCUSDT");
		if (TextUtils.isEmpty(symbol) || !ContractInfoController.getInstance().checkContractIsExist(symbol)) {
			symbol = "BTCUSDT";
		}
	...
	private String getOrderbookTopic(String symbol, int depth) {
		...
		return String.format("btc/orderBook.%s.%d", symbol, depth);
	}

	private String getTradeDetailTopic(String symbol) {
		//	spot/tradeList.{symbol}
		return String.format("btc/tradeList.%s", symbol);
	}

	private String getTickerTopic(String symbol) {
		return String.format("btc/ticker.%s", symbol);
	}
	...
	public static NewContractBtcWebsocket getInstance() {
		if (mInstance == null) {
			synchronized (NewContractBtcWebsocket.class) {
				if (mInstance == null) {
					mInstance = new NewContractBtcWebsocket();
				}
			}
		}
		return mInstance;
