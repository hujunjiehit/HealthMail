 ...
 * on 2020-01-15
 */
public class NewSpotWebsocket extends BaseWebSocket implements WebsocketOperatiron {

	private static final String WS_TRADEPAIR_KEY = "spotWsTradePair";

	private static volatile NewSpotWebsocket mInstance;
	...
	private NewSpotWebsocket() {

		tickerData = new WsMarketData();

		symbol = MMKV.defaultMMKV().decodeString(WS_TRADEPAIR_KEY, "BTCUSDT");
		if (TextUtils.isEmpty(symbol) || TradePairInfoController.getInstance().queryDataById(symbol) == null) {
			symbol = "BTCUSDT";
		}
		...
		orderBookSubscriber = new WsSubscriber<WsOrderbook>(getOrderbookTopic(symbol, defaultDepth)) {
			...
			protected void onMessage(List<WsOrderbook> data, String action) {
				...
				if (BuildConfig.DEBUG && action.equals("update")) {
					if (BigDecimalUtils.compareZero(BigDecimalUtils.subtract(String.valueOf(data.get(0).getTimestamp()), lastTs)) < 0) {
						DLog.e("websocketts", "ts = " + BigDecimalUtils.subtract(String.valueOf(data.get(0).getTimestamp()), lastTs));
					}
					lastTs = String.valueOf(data.get(0).getTimestamp());
				}
			}
		};
	...
	private String getOrderbookTopic(String symbol, int depth) {
		...
		return String.format("spot/orderBook.%s.%d", symbol, depth);
	}

	private String getTradeDetailTopic(String symbol) {
		//	spot/tradeList.{symbol}
		return String.format("spot/tradeList.%s", symbol);
	}

	private String getTickerTopic(String symbol) {
		return String.format("spot/ticker.%s", symbol);
	}
	...
	public static NewSpotWebsocket getInstance() {
		if (mInstance == null) {
			synchronized (NewSpotWebsocket.class) {
				if (mInstance == null) {
					mInstance = new NewSpotWebsocket();
				}
			}
		}
		return mInstance;
