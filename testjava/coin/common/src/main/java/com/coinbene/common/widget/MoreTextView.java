...
public class MoreTextView extends TextView {
    ...
    // 默认收起文字
    private static final String DEFAULT_FOLD_TEXT = "";
    // 默认展开文字
    private static final String DEFAULT_UNFOLD_TEXT = "";
    // 默认固定行数
    private static final int DEFAULT_FOLD_LINE = 5;
    // 默认收起和展开文字颜色
    private static final int DEFAULT_TAIL_TEXT_COLOR = 0;
    // 默认是否可以再次收起
    ...
    public MoreTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mFoldText = a.getString(R.styleable.MoreTextView_foldText);
        if (null == mFoldText) {
            mFoldText = DEFAULT_FOLD_TEXT;
        }
        mUnFoldText = a.getString(R.styleable.MoreTextView_unFoldText);
        if (null == mUnFoldText) {
            mUnFoldText = DEFAULT_UNFOLD_TEXT;
        }
        mFoldLine = a.getInt(R.styleable.MoreTextView_foldLine, DEFAULT_FOLD_LINE);
        if (mFoldLine < 1) {
            throw new RuntimeException("foldLine must not less than 1");
        }
        mTailColor = a.getColor(R.styleable.MoreTextView_tailTextColor, DEFAULT_TAIL_TEXT_COLOR);
        mCanFoldAgain = a.getBoolean(R.styleable.MoreTextView_canFoldAgain, DEFAULT_CAN_FOLD_AGAIN);
