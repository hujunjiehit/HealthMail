...
public class ClosePositionBtcActivity extends CoinbeneBaseActivity {
	...
	private TextWatcher watcherCont;
	...
	public static void startMe(Context context, String contractName, String side, String avlPosition, String avgPrice) {
		...
		Intent intent = new Intent(context, ClosePositionBtcActivity.class);
	...
	private void listener() {
		...
		inputNum.setHint(String.format("%s(%s)",getString(R.string.input_num_hint),getString(R.string.number)));
...
//				inputNum.setSeparatorText(watcherCont);
		...
		inputNum.setMinPriceChange(TradeUtils.getContractBtcMultiplier(table));
	...
	private void calEstimatedValue() {
		if (table != null && !BigDecimalUtils.isEmptyOrZero(inputNum.getText())) {
			if (TradeUtils.isContractBtc(table)) {
	...
	private void changeToCloseCont(int progress) {
		...
		inputNum.setText(bdPg.divide(onHundred).multiply(bdAvl).setScale(0, BigDecimal.ROUND_DOWN).toPlainString());
	...
	private void calculationReturn() {
		if (BigDecimalUtils.isEmptyOrZero(inputPrice.getText())) {
			tvProfit.setText(String.format("%s-- BTC", getString(R.string.expected_return)));
			return;
		}
		...
		if (contractName.contains("BTC")) {
			...
			tvProfit.setText(String.format("%s%s", getString(R.string.expected_return), String.format("%s BTC", CalculationUtils.calculateEthExpectedReturn(
					side, avgPrice,
					inputPrice.getText(),
					inputNum.getText(), table.multiplier,
					4)
			)));
		}
	}
	...
	private void changeToSeekBar() {
		...
		int persent = BigDecimalUtils.divideHalfUp(inputNum.getText(), avlPosition, 2);
		if (persent < 100) {
	...
	private void getData() {
		...
		tvTitle.setText(String.format("%s %s", String.format(getString(R.string.forever_no_delivery), contractName), placeOrderTitle));
		tvAvlCloseValue.setText(String.format(getString(R.string.future_24vol_unit_label), avlPosition));
	...
	public void onClick(View v) {
		...
		if (id == R.id.iv_close) {
			...
			contractPlaceOrderModel.setNumber(inputNum.getText());
...
//			buildTitle(currentPriceType == 0 ? getString(R.string.fixed_price) : getString(R.string.market_price), placeOrderTitle, contractName)
//					.buildPrice(PrecisionUtils.appendZero(inputPrice.getText(), table.precision))
//					.buildNumber(String.format("%s%s",inputNum.getText(),getString(R.string.number)))
...
//					.show();
	...
	private void closeOrder(int tradeDirection, String price, String number, String contractName) {
		...
		OkGo.<ContractPlaceOrderModel>post(Constants.TRADE_CLOSE_ORDER).params(params).tag(this).execute(new DialogCallback<ContractPlaceOrderModel>(this) {
			@Override
			public void onSuc(Response<ContractPlaceOrderModel> response) {

				ToastUtil.show(R.string.buyorsell_success);
				DataCollectionHanlder.getInstance().putClientData(DataCollectionHanlder.appPlaceOrder, new DataCollectionModel(response.body().getData().getOrderId(), DataCollectionModel.CONTRACT,
	...
	private void init() {
		...
		buyBind = new OrderBookBidsItemBinder(false, greenColor, redColor, isRedRise, amountTextColor, Constants.CONTRACT_TYPE_BTC);
		buyBind.setUsedKline(false);
		...
		sellBind = new OrderBookAsksItemBinder(true, greenColor, redColor, isRedRise, amountTextColor, Constants.CONTRACT_TYPE_BTC);
