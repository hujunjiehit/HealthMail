...
public class ContractCenterBtcLayout extends LinearLayout implements View.OnClickListener {
	...
	ContractCenterBtcLayout rootLayout;
	...
	public ContractBtcOrderBookLayout contractBtcOrderBookLayout;
	...
	private int currentIndex = Constants.INDEX_OPEN;

	private int currentDisplay = Constants.DISPLAY_ORDER_LIST;
	...
	private ContractInfoTable contractInfoTable;
	...
	public void setContractInfoTable(ContractInfoTable contractBtcInfoTable) {
		this.contractInfoTable = contractBtcInfoTable;
	...
	private void updateContractInfo() {
		...
		inputNum.setHint(String.format("%s(%s)", getResources().getString(R.string.input_num_hint), getResources().getString(R.string.number)));
		inputNum.setMinPriceChange(TradeUtils.getContractBtcMultiplier(contractInfoTable));
		...
		calEnstimatedOneValue();
	}

	private void calEnstimatedOneValue() {
		if (contractInfoTable != null) {
			if (TradeUtils.isContractBtc(contractInfoTable)) {//btc是等于  其他是约等于
				tvEstimatedValueOneValue.setText(String.format("%s",
						"1USDT"));
			} else {
//				tvEstimatedValueOneValue.setText(String.format("%s",
//						CalculationUtils.getBtcContractEstimatedOne(contractInfoTable,
//								NewContractBtcWebsocket.getInstance().getTickerData() == null
//										? "0" :
//										NewContractBtcWebsocket.getInstance().getTickerData().getLastPrice())
//				));
			}
	...
	public ContractCenterBtcLayout(Context context) {
		super(context);
	}

	public ContractCenterBtcLayout(Context context, AttributeSet attrs) {
		super(context, attrs);

	}

	public ContractCenterBtcLayout(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void initView() {
		contractBtcOrderBookLayout.setTvMarkPrice(tvMarketPriceValue);
		tvLever.setText(String.format("%dX", currentLever));
		tvAvlLongNumber.setText(String.format("%d%s", 0, getResources().getString(R.string.number)));
		tvAvlShortNumber.setText(String.format("%d%s", 0, getResources().getString(R.string.number)));
		rbOpenLong.setChecked(true);
		rgOpen.setOnCheckedChangeListener((group, checkedId) -> {
			if (checkedId == R.id.rb_open_long) {
				currentIndex = Constants.INDEX_OPEN;
				calculationAvlOpen();
				...
				currentIndex = Constants.INDEX_CLOSE;
				setAvlClose();
	...
	private void setAvlClose() {
		...
		tvCloseShortNumber.setText(String.format("%s%s", avlCloseAccount[1], getResources().getString(R.string.number)));
		tvCloseLongNumber.setText(String.format("%s%s", avlCloseAccount[0], getResources().getString(R.string.number)));

//        tvAvlLongNumber.setText(String.format("%d%s", avlCloseAccount[1], getResources().getString(R.string.number)));
//        tvAvlShortNumber.setText(String.format("%d%s", avlCloseAccount[0], getResources().getString(R.string.number)));
	}
	...
	public void setAmountUnit(String baseAsset) {
		...
		tvOrderListVol.setText(
				String.format("%s(%s)", getResources().getString(R.string.trade_vol), getResources().getString(R.string.number)));
	}
	...
	private void initListener() {
		...
		contractBtcOrderBookLayout.setOrderItemClick(price -> {
			if (curFixPriceType != Constants.MARKET_PRICE) {
				inputPrice.setText(price);
				inputPrice.setSelection(inputPrice.getText().toString().length());
//                calculationAvlOpen();
			}
		});

		inputPrice.setOnFocusChangeListener((v, hasFocus) -> {
			if (curFixPriceType != Constants.MARKET_PRICE)
				if (hasFocus) {
		...
		numTextWatcher = new BaseTextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before, int count) {
				...
				if (contractInfoTable != null && !BigDecimalUtils.isEmptyOrZero(inputNum.getText())) {
//					if (TradeUtils.isContractBtc(contractInfoTable)) {
//						tvEstimatedValue.setText(String.format("=%s %s",
//								CalculationUtils.getBtcContractEstimated(
//										contractInfoTable,
...
//				inputNum.setSeparatorText(numTextWatcher);
		...
		inputNum.setOnFocusChangeListener((v, hasFocus) -> {
			if (hasFocus) {
				rlNumber.setBackground(getResources().getDrawable(R.drawable.shape_edit_focused_background));
			} else {
				rlNumber.setBackground(getResources().getDrawable(R.drawable.shape_edit_normal_background));
			}
		});
		...
		mTypeSelectorDialog.setSelectListener((data, positon) -> {
			...
			contractBtcOrderBookLayout.setCurrentDisplay(positon);
		...
		mRgTab.setOnCheckedChangeListener((group, checkedId) -> {
			if (checkedId == R.id.rb_order_book) {
				...
				currentDisplay = Constants.DISPLAY_ORDER_LIST;
				...
				currentDisplay = Constants.DISPLAY_ORDER_DETAIL;
	...
	public void changeTabToClosePosition(int currentIndex) {
		...
		if (currentIndex == Constants.INDEX_OPEN) {
			rbOpenLong.setChecked(true);
		} else
...
//        }
		...
		inputNum.setText("");
		if (currentIndex == Constants.INDEX_OPEN) {//开仓
...
//                seekBar.setSecondTrackColor(getContext().getResources().getColor(R.color.ma5));
	...
	public void setOrderListData(List<OrderModel> buyList, List<OrderModel> sellList, WsMarketData tickerData) {
		...
		contractBtcOrderBookLayout.setOrderListData(buyList, sellList, tickerData);
		...
		if (tickerData != null && !TextUtils.isEmpty(tickerData.getLastPrice())) {
			...
			if (!TextUtils.isEmpty(tickerData.getLastPrice())
					&& !TextUtils.isEmpty(firstAndNewPrice[2])
					&& !firstAndNewPrice[2].equals(tickerData.getLastPrice())) {
				firstAndNewPrice[2] = tickerData.getLastPrice();
				post(() -> {
					if (!TradeUtils.isContractBtc(contractInfoTable))
						calEnstimatedOneValue();
	...
	public void setOrderDetailData(List<WsTradeList> orderDetailList) {
		contractBtcOrderBookLayout.setOrderDetailData(orderDetailList);
	}
	...
	public void onClick(View v) {
		...
		int id = v.getId();
		if (id == R.id.buy_tv) {
			if (currentIndex == Constants.INDEX_OPEN) {// 买入开多
			...
			gotoLoginOrLock();
		} else if (id == R.id.tv_close_long_number) {//                if(avlCloseAccount[0]!=0){
			inputNum.setText(avlCloseAccount[0]);
//                }
		} else if (id == R.id.tv_close_short_number) {//                if(avlCloseAccount[1]!=0){
			inputNum.setText(avlCloseAccount[1]);
//                }
		} else if (id == R.id.ll_lever) {
			//如果用户当前已经有委托  则不允许切换
	...
	public void checkAndShowTradeDialog(int tradeDirection, String fixedStr, String buyOrSell, String price, String number, String symble, int precision, int curFixPriceType, boolean needCheck) {
		...
		if (needCheck) {
			...
			if (BigDecimalUtils.isEmptyOrZero(inputNum.getText().toString())) {
			...
			if (tradeDirection == Constants.TRADE_OPEN_LONG) {
				...
				if (BigDecimalUtils.isGreaterThan(inputNum.getText(), maxOrderAccount)) {
			...
			if (tradeDirection == Constants.TRADE_OPEN_SHORT) {
				...
				if (BigDecimalUtils.isEmptyOrZero(avlOpenAccount[1]) || BigDecimalUtils.isGreaterThan(inputNum.getText(), String.valueOf(avlOpenAccount[1]))) {
					ToastUtil.show(R.string.available_balance_Insufficient);
					return;
				}
				if (curFixPriceType == Constants.FIXED_PRICE) {
			...
			if (tradeDirection == Constants.TRADE_CLOSE_LONG) {
				if (BigDecimalUtils.isEmptyOrZero(avlCloseAccount[0]) || BigDecimalUtils.isGreaterThan(inputNum.getText(), avlCloseAccount[0])) {
			...
			if (tradeDirection == Constants.TRADE_CLOSE_SHORT) {
				if (BigDecimalUtils.isEmptyOrZero(avlCloseAccount[1]) || BigDecimalUtils.isGreaterThan(inputNum.getText(), avlCloseAccount[1])) {
					ToastUtil.show(R.string.close_amount_insufficient_tip);
					return;
				}
			}
...
//			placeOrder(tradeDirection, curFixPriceType, curHighLeverEntrust, price, inputNum.getText(), symble);
//			return;
//		}
//		if (tradeDialog == null) {
...
////					placeOrder(tradeDirection, priceType, curHighLeverEntrust, price, inputNum.getText(), contractName);
////				}
//			});
//		}
...
//				.buildNumber(number + getResources().getString(R.string.number))
...
//				.show();
	...
	private void placeOrder(int tradeDirection, int priceType, int curHighLeverEntrust, String price, String number, String contractName) {
		if (centerViewGroupListener != null) {
			centerViewGroupListener.clickPlaceOrder(tradeDirection, priceType, curHighLeverEntrust, currentLever,
					price, number, contractName, etStopProfit.getText(), etStopLoss.getText());
		}
	}
	...
	public void setLever(int lever) {
		if (lever == 0) {
			...
			isHaveLever = true;
			currentLever = lever;
			ContractInfoController.getInstance().updataContractLever(contractInfoTable.name, currentLever);
...
//            calculationAvlOpen();
	...
	public void calculationAvlOpen() {
		if (contractInfoTable != null && contractConfigTable != null && CommonUtil.isLoginAndUnLocked() && isGetUserUseLever && currentIndex == Constants.INDEX_OPEN) {
			//BTC可开计算
			if (contractInfoTable.name.contains("BTC")) {
				avlOpenAccount = CalculationUtils.calculationBtc(curFixPriceType, inputPrice.getText().toString(),
						firstAndNewPrice[0],
						firstAndNewPrice[1],
						firstAndNewPrice[2],
						firstAndNewPrice[3],
			...
			if (BigDecimalUtils.isThanOrEqual(avlOpenAccount[0], CalculationUtils.maxAvlOpenAmount)) {
				...
				tvAvlLongNumber.setText(String.format("%s%s%s", avlOpenAccount[0], "+", getResources().getString(R.string.number)));
			} else {
				tvAvlLongNumber.setText(String.format("%s%s", avlOpenAccount[0], getResources().getString(R.string.number)));
			}
			if (BigDecimalUtils.isThanOrEqual(avlOpenAccount[1], CalculationUtils.maxAvlOpenAmount)) {
				...
				tvAvlShortNumber.setText(String.format("%s%s%s", avlOpenAccount[1], "+", getResources().getString(R.string.number)));
			} else {
				tvAvlShortNumber.setText(String.format("%s%s", avlOpenAccount[1], getResources().getString(R.string.number)));
			}
		}
	}
	...
	public void setUserData(String availableBalance, String longQuantity, String shortQuantity) {
		...
		this.avlCloseAccount[0] = longQuantity;
		this.avlCloseAccount[1] = shortQuantity;

		if (currentIndex == Constants.INDEX_CLOSE) {
			setAvlClose();
		}
...
//		inputPrice.setText("");
	...
	public void setOrderType(int curDisplayType) {
		contractBtcOrderBookLayout.setCurDisplayType(curDisplayType);
	}
