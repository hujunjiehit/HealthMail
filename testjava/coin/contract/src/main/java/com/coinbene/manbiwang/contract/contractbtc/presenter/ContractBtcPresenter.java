package com.coinbene.manbiwang.contract.contractbtc.presenter;
 ...
 */
public class ContractBtcPresenter implements ContractInterface.Presenter {
	...
	private NewContractBtcWebsocket.OrderbookDataListener mOrderBookDataListener;
	private NewContractBtcWebsocket.TradeListDataListener mTradeDetailListener;
	...
	public ContractBtcPresenter(ContractInterface.View view, String contractName) {
	...
	public void subAll() {
		...
		if (mOrderBookDataListener == null) {
			mOrderBookDataListener = new NewContractBtcWebsocket.OrderbookDataListener() {
				@Override
		...
		if (mTradeDetailListener == null) {
			mTradeDetailListener = new NewContractBtcWebsocket.TradeListDataListener() {
				@Override
	...
	public void openContract() {
		//flag：1打开，0关闭（用不上）
		OkGo.<BaseRes>post(Constants.OPEN_BTC_CONTRACT).params("flag", "1").tag(this).execute(new NewJsonSubCallBack<BaseRes>() {
			@Override
			public void onSuc(Response<BaseRes> response) {
				if (response.isSuccessful()) {
					SpUtil.setContractStatus(CBRepository.getContext(), SpUtil.BTC_CONTRACT_STATUS, true);
				}
			}
	...
	public void agreeProtocol(String key, String value) {
		...
		OkGo.<BaseRes>post(Constants.UPDATE_USER_CONFIG).params(params).tag(this).execute(new NewJsonSubCallBack<BaseRes>() {
			@Override
			public void onSuc(Response<BaseRes> response) {
				if (response.isSuccessful()) {
	...
	public void setOrderType(int orderType) {
		...
		if (orderType == Constants.DISPLAY_ORDER_LIST) {
			NewContractBtcWebsocket.getInstance().pullOrderListData(contractName);
		} else {
			NewContractBtcWebsocket.getInstance().pullTradeDetailData(contractName);
		}
	...
	public void getFundingTime() {
		OkGo.<FundingTimeModel>get(Constants.MARKET_CONTRACT_FUNDING_TIME).tag(this).execute(new NewJsonSubCallBack<FundingTimeModel>() {
			@Override
	...
	public void getCurLeverage() {
		OkGo.<ContractLeverModel>get(Constants.TRADE_CUR_LEVERAGE).tag(this).params("symbol", contractName).execute(new NewJsonSubCallBack<ContractLeverModel>() {
			@Override
	...
	public void cancelOrder(String orderId, Activity activity) {
		...
		OkGo.<ContractPlaceOrderModel>post(Constants.TRADE_CANCEL_ORDER).tag(this).params("orderId", orderId).execute(new DialogCallback<ContractPlaceOrderModel>(activity) {
			@Override
...
//                ToastUtil.show(R.string.cancel_order_fail);
	...
	public void getPositionlist() {
		OkGo.<ContractPositionListModel>get(Constants.ACCOUNT_POSITION_LIST).tag(this).params("symbol", contractName).execute(new NewJsonSubCallBack<ContractPositionListModel>() {
			@Override
	...
	public void getContractAccountInfo() {
		OkGo.<ContractAccountInfoModel>get(Constants.CONTRACT_ACCOUNT_INFO).tag(this).params("symbol", contractName).execute(new NewJsonSubCallBack<ContractAccountInfoModel>() {
			@Override
	...
	public void getCurrentOrderList() {
		...
		OkGo.<CurrentDelegationModel>get(Constants.CONTRACT_CURRENT_DELEGATION).params(params).tag(this).execute(new NewJsonSubCallBack<CurrentDelegationModel>() {
			@Override
	...
	public void setPositionMode(String symbol, String mode) {
		...
		OkGo.<BaseRes>post(Constants.CONTRACT_POSTIONMODE_CHANGE).tag(this).params(params).execute(new NewJsonSubCallBack<BaseRes>() {
			@Override
			public void onSuc(Response<BaseRes> response) {
...
//			long startTime = System.currentTimeMillis();
			...
			OkGo.<ContractPlaceOrderModel>post(Constants.TRADE_PLACE_ORDER).params(params).tag(this).execute(new DialogCallback<ContractPlaceOrderModel>(activity) {
				@Override
				public void onSuc(Response<ContractPlaceOrderModel> response) {
...
//			long startTime = System.currentTimeMillis();
			OkGo.<ContractPlaceOrderModel>post(Constants.TRADE_CLOSE_ORDER).params(params).tag(this).execute(new DialogCallback<ContractPlaceOrderModel>(activity) {
				@Override
				public void onSuc(Response<ContractPlaceOrderModel> response) {
