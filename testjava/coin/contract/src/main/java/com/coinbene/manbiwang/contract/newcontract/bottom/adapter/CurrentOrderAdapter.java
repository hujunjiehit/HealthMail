 ...
 * on 2019-09-09
 */
public class CurrentOrderAdapter extends BaseQuickAdapter<CurrentDelegationModel.DataBean.ListBean, BaseViewHolder> {
	...
	public CurrentOrderAdapter() {
		super(R.layout.contract_item_current_order);
	}
	...
	protected void convert(@NonNull BaseViewHolder helper, CurrentDelegationModel.DataBean.ListBean item) {
		...
		if (TradeUtils.isUsdtContract(item.getSymbol())) {
			...
			//btc合约
			helper.setText(R.id.textView2, String.format(entrustUnitCoin, mContext.getString(R.string.number)));
			helper.setText(R.id.textView5, String.format(dealUnitCoin, mContext.getString(R.string.number)));

			//合约类型
			helper.setText(R.id.current_delegation_contracts, String.format(symbleStr, item.getSymbol()));
			//委托量
			helper.setText(R.id.current_delegation_vol, item.getQuantity());
			//成交量
			helper.setText(R.id.current_clinch_vol, item.getFilledQuantity());
		}
