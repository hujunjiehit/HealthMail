 ...
 * 合约历史委托适配器
 */
public class HistoryOrderAdapter extends BaseQuickAdapter<HistoryDelegationModel.DataBean.ListBean, BaseViewHolder> {
	...
	public HistoryOrderAdapter() {
		super(R.layout.contract_item_history_order);
	}
	...
	protected void convert(@NonNull BaseViewHolder helper, HistoryDelegationModel.DataBean.ListBean item) {
		...
		if (TradeUtils.isUsdtContract(item.getSymbol())) {
			...
			helper.setText(R.id.textView10, String.format(entrustUnitCoin, TradeUtils.getContractUsdtUnit(table)));
			helper.setText(R.id.textView13, String.format(dealUnitCoin, TradeUtils.getContractUsdtUnit(table)));

			//委托量
			helper.setText(R.id.history_delegation_vol, TradeUtils.getContractUsdtUnitValue(item.getQuantity(), table));

			//成交量
			helper.setText(R.id.history_deal_vol, TradeUtils.getContractUsdtUnitValue(item.getFilledQuantity(), table));

			//合约类型
			helper.setText(R.id.history_delegation_contrancts, String.format(symbleStr, table.baseAsset));
		} else {
			//btc合约
			helper.setText(R.id.textView10, String.format(entrustUnitCoin, mContext.getString(R.string.number)));
			helper.setText(R.id.textView13, String.format(dealUnitCoin, mContext.getString(R.string.number)));
