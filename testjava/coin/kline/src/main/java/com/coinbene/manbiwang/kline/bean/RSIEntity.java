...
//        this(kLineBeens, n, 100);
     ...
     * @param defult     不足N日时的默认值
     */
    public void RSIDeal(List<KLineBean> kLineBeens, int n, float defult) {
        RSIs = new ArrayList<>();
        float sum = 0.0f;
        float dif = 0.0f;
        float rs = 0.0f;
...
//                    rsi = 100 - c;
    ...
    private Float[] getAAndB(Integer a, Integer b, List<KLineBean> kLineBeens) {
        if (a < 0) {
            a = 0;
        }
        float sum = 0.0f;
        ...
        for (int i = a; i <= b; i++) {
            if (i > a) {
                closeT = (float) kLineBeens.get(i).close;
                closeY = (float) kLineBeens.get(i - 1).close;
