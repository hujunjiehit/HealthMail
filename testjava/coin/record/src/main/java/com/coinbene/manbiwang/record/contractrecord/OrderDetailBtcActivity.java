...
import com.coinbene.manbiwang.record.contractrecord.adapter.DetailAdapter;
...
import static com.coinbene.common.Constants.CONTRACT_TRANSACTION_DETAIL;


@Route(path = RouteHub.Record.contractBtcOrderDetail)
public class OrderDetailBtcActivity extends CoinbeneBaseActivity {
	...
	public static void startMe(Context context, HistoryDelegationModel.DataBean.ListBean bean) {
		Intent intent = new Intent(context, OrderDetailBtcActivity.class);
	...
	private void init() {
		...
		price.setText(String.format("%s/%s", bean.getAveragePrice(), Constants.TYPE_PLAN_MARKET.equals(bean.getOrderType()) ? "--" : bean.getOrderPrice()));
		free.setText(bean.getFee());
		vol.setText(String.format("%s/%s", bean.getFilledQuantity(), bean.getQuantity()));
		worth.setText(bean.getOrderValue());
		setTagBackGround(direction, bean.getDirection());

		contract.setText(String.format(getResources().getString(R.string.forever_no_delivery), bean.getSymbol()));
	...
	private void getDetail(String id, String direction) {
		...
		OkGo.<ContractDetailMode>get(CONTRACT_TRANSACTION_DETAIL).params(params).tag(this).execute(new NewDialogJsonCallback<ContractDetailMode>(this) {
			@Override
