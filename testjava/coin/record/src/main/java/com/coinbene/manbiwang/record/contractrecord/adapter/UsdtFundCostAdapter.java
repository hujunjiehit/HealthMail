 ...
 * 资金费用
 */
public class UsdtFundCostAdapter extends BaseAdapter<FundCostModel.DataBean.ListBean> {
	...
	protected void convert(BaseViewHolder holder, int position, FundCostModel.DataBean.ListBean item) {
		...
		//合约类型
		holder.setText(R.id.fund_cost_contracts, String.format(symbleStr, table.baseAsset));

		//时间
		holder.setText(R.id.fund_cost_time, format.format(new Date(Long.valueOf(item.getTime()))));

		holder.setText(R.id.textView22, String.format("%s(%s)", vol, TradeUtils.getContractUsdtUnit(table)));
		//成交量
		holder.setText(R.id.fund_cost_vol, String.format("%s", TradeUtils.getContractUsdtUnitValue(item.getPosition(), table)));
	...
	public BaseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
		...
		vol = parent.getResources().getString(R.string.vol);
