...
public class OrderStatuFragment extends BaseFragment {
    ...
    public static OrderStatuFragment newInstance(int orderType) {
        Bundle args = new Bundle();
        args.putInt("orderType", orderType);
        OrderStatuFragment fragment = new OrderStatuFragment();
    ...
    private OtcOrderListBinder.ItemClickListener itemClickListener = adID ->
            OtcOrderDetailActivity.startMe(getActivity(), adID, orderType);
    ...
    private void getOrderList() {
        ...
        OkGo.<OrderListModel>get(Constants.OTC_GET_ORDER_List).params(params).tag(this).execute(new NewJsonSubCallBack<OrderListModel>() {
            @Override
            public void onSuc(Response<OrderListModel> response) {
                ...
                if (OrderStatuFragment.this.isDetached() || empty_layout == null || rl_list_order == null) {
                    return;
                }
                OrderListModel t = response.body();
                if (t != null && t.getData() != null && t.getData().getResult() != null && t.getData().getResult().size() > 0) {

                    List<OrderListModel.DataBean.ResultBean> result = t.getData().getResult();

                    empty_layout.setVisibility(View.GONE);
                    rl_list_order.setVisibility(View.VISIBLE);
                    pageSize = response.body().getData().getPageSize();
