...
/**
 */
public class OtcPayDialogAdapter extends RecyclerView.Adapter<OtcPayDialogAdapter.MyHolder> {
    ...
    public int getSelectPayType() {
        return lists == null || lists.size() == 0 ? 0 : lists.get(selectPosition).payId;
    }
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        if(lists.get(position).payTypeId == 1){
            holder.tvCoin.setText(lists.get(position).bankName);
