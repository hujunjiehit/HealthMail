...
public class AdDetailPayBinder extends ItemViewBinder<SellerAdModel.DataBean.ListBean.PayWayListBean, AdDetailPayBinder.ViewHolder> {
    ...
    @Override
    protected void onBindViewHolder(@NonNull ViewHolder holder, @NonNull SellerAdModel.DataBean.ListBean.PayWayListBean bindInfo) {
        if (bindInfo.getPayTypeId() == TYPE_BANK) {
            holder.bind_info_tv.setText(bindInfo.getBankName());
            holder.bind_info_value_tv.setText(bindInfo.getBankAccount());
        } else if (bindInfo.getPayTypeId() == TYPE_WEIXIN) {
        ...
        } else if (bindInfo.getPayTypeId() == TYPE_ALIPAY) {
