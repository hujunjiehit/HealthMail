...
import com.github.fujianlian.klinechart.view.FenshiAnimView;
...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
	...
	private static int MARGIN_LENGTH = 200; //屏幕右边显示最新价的宽度
	...
	private Paint mTextDefautePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	...
	private Paint mFenshiBgPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
	...
	//是否是红涨绿跌，默认红色代表涨，绿色代表跌
	private boolean isRedRise = true;
	private boolean isFirstLoad = true;

	private FenshiAnimView mFenshiAnimView;
	...
	private void init() {
		...
		mSelectorFramePaint.setColor(getContext().getResources().getColor(R.color.chart_selector));

		mFenshiBgPaint.setStyle(Paint.Style.FILL);
		mFenshiBgPaint.setColor(getContext().getResources().getColor(R.color.res_blue));

		mLastPricePaint.setColor(ContextCompat.getColor(getContext(), R.color.chart_line));
		effects = new DashPathEffect(new float[]{16, 6}, 1);

		mFenshiAnimView = new FenshiAnimView(getContext());
		FrameLayout.LayoutParams viewParameter = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
		addView(mFenshiAnimView, viewParameter);

		logoIcon = new ImageView(getContext());
		logoIcon.setImageDrawable(getContext().getResources().getDrawable(R.drawable.icon_kline_logo));
	}

	@Override
	protected void onDetachedFromWindow() {
		super.onDetachedFromWindow();
		if (mFenshiAnimView != null) {
			mFenshiAnimView.dismissCurrentAnimation();
			mFenshiAnimView = null;
		}
		logoIcon = null;
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		//MARGIN_LENGTH = mWidth/5;

		//列宽度
		MARGIN_LENGTH = mWidth / 5 - mChildPadding / 2;
		//右边移动最大设置一个列宽度
		setOverScrollRange(MARGIN_LENGTH);
	...
	private void addIcon() {
		RelativeLayout.LayoutParams logoParams =
				new RelativeLayout.LayoutParams(ViewUtil.Dp2Px(getContext(), 124), ViewUtil.Dp2Px(getContext(), 24));
		logoParams.leftMargin = ViewUtil.Dp2Px(getContext(), 5);
		logoParams.topMargin = mVolRect.top - mMainRect.top - ViewUtil.Dp2Px(getContext(), 24);

		removeViewInLayout(logoIcon);
		addView(logoIcon, logoParams);
	}
	...
	public float getLastPriceFormY(float y) {
		return mMainMaxValue - ((y - mMainRect.top) / mMainScaleY);
	}

	public void setClickY(float y) {
		this.clickY = y;
	}
	...
	private void drawK(Canvas canvas) {
		...
		Log.e("testDrawK", mStartIndex + " mStopIndex ==> " + mStopIndex);
		...
		//画选择线
		if (isLongPress) {
			if (mSelectedIndex >= getAdapter().getCount()) {
				return;
			}
	 ...
	 * @param y
	 */
	private void drawSelectCircle(Canvas canvas, float x, float y) {
		//绘制大圆
		float r1 = ViewUtil.Dp2Px(getContext(), 4);
		canvas.drawRoundRect(x - r1/mScaleX, y - r1, x + r1/mScaleX, y + r1, r1/mScaleX, r1 , mSelectedYLinePaint);

		//绘制小圆
		float r2 = ViewUtil.Dp2Px(getContext(), 1);
		canvas.drawRoundRect(x - r2/mScaleX, y - r2, x + r2/mScaleX, y + r2, r2/mScaleX, r2 , mTextPaint);
	}
	...
	private void drawText(Canvas canvas) {
		...
		//--------------画上方k线图的值-------------
		if (mMainDraw != null) {
			canvas.drawText(formatValue(mMainMaxValue), mWidth - calculateWidth(formatValue(mMainMaxValue)), baseLine + mMainRect.top, mTextDefautePaint);
			canvas.drawText(formatValue(mMainMinValue), mWidth - calculateWidth(formatValue(mMainMinValue)), mMainRect.bottom - textHeight + baseLine, mTextDefautePaint);
			...
			for (int i = 1; i < mGridRows; i++) {
				...
				canvas.drawText(text, mWidth - calculateWidth(text), fixTextY(rowSpace * i + mMainRect.top), mTextDefautePaint);
			}
		}
		//--------------画中间子图的值-------------
		if (mVolDraw != null) {
			canvas.drawText(mVolDraw.getValueFormatter().format(mVolMaxValue),
					mWidth - calculateWidth(formatValue(mVolMaxValue)), mMainRect.bottom + baseLine, mTextDefautePaint);
		...
		if (mChildDraw != null) {
			canvas.drawText(mChildDraw.getValueFormatter().format(mChildMaxValue),
					mWidth - calculateWidth(formatValue(mChildMaxValue)), mVolRect.bottom + baseLine, mTextDefautePaint);
		...
		for (int i = 1; i < mGridColumns; i++) {
			...
			if (translateX >= startX && translateX <= stopX) {
				...
				canvas.drawText(text, columnSpace * i - mTextPaint.measureText(text) / 2, y, mTextDefautePaint);
			}
		}
		...
		if (translateX >= startX && translateX <= stopX) {
			//显示第一个时间
			String text = formatDateTime(getAdapter().getDate(mStartIndex));
			canvas.drawText(text, 0 - mTextPaint.measureText(text) / 2, y, mTextDefautePaint);
		}
		translateX = xToTranslateX(mWidth);
		if (translateX >= startX && translateX <= stopX) {
			//显示最后一个时间
			String text = formatDateTime(getAdapter().getDate(mStopIndex));
			canvas.drawText(text, mWidth - mTextPaint.measureText(text) / 2, y, mTextDefautePaint);
		}
		...
		if (isLongPress) {
			...
			String text;
			y = clickY;
			text = String.valueOf(getLastPriceFormY(clickY));
			float x;
	...
	private void drawLastPrice(Canvas canvas) {
		int index = mAdapter.getCount() - 1;
		IKLine point = (IKLine) getItem(index);

		float x = translateXtoX(getX(index));
		float y = getMainY(point.getClosePrice());

		Log.d("testAnim", "drawLastPrice:" + point.getClosePrice());
		Log.d("testAnim", "X:" + x);
		Log.d("testAnim", "Y:" + y);

		if (canvas != null) {
			float leftLength = mWidth - x;
			int padding = 4;

			String text = formatValue(point.getClosePrice());

			float textWidth = mLastPricePaint.measureText(text);
			float textHeight = mLastPricePaint.getFontMetrics().bottom - mLastPricePaint.getFontMetrics().top + mLastPricePaint.getFontMetrics().leading;

			if (leftLength - textWidth - padding >= ViewUtil.Dp2Px(getContext(), 5)) {
				//绘制虚线
				mLastPricePaint.setStyle(Paint.Style.STROKE);
				mLastPricePaint.setPathEffect(effects);
				canvas.drawLine(x + ViewUtil.Dp2Px(getContext(), 5), y, x + leftLength - textWidth - padding, y, mLastPricePaint);
				...
				//绘制最新价后面的背景
				Path path = new Path();
				float beginX = x + leftLength - textWidth - padding;
				path.moveTo(beginX, y - textHeight / 2);
				path.lineTo(beginX + textWidth, y - textHeight / 2);
				path.lineTo(beginX + textWidth, y + textHeight / 2);
				path.lineTo(beginX, y + textHeight / 2);
				path.close();
				canvas.drawPath(path, mSelectPointPaint);

				//绘制最新价
				canvas.drawText(text, x + leftLength - textWidth - padding, y + textHeight / 4, mLastPricePaint);

				if (mainDraw.isLine()) {
					//分时动画
					mFenshiAnimView.updateLocation(x, y);
				} else {
					mFenshiAnimView.dismissCurrentAnimation();
				}
			} else {
				...
				x = mWidth - MARGIN_LENGTH;

				//绘制左边虚线
				mLastPricePaint.setStyle(Paint.Style.STROKE);
				mLastPricePaint.setPathEffect(effects);
				canvas.drawLine(1, y, x - textWidth / 2 - w1 - w2, y, mLastPricePaint);

				Path path = new Path();
				path.moveTo(x - textWidth / 2 - w1 - w2, y);
				...
				canvas.drawPath(path, mSelectPointPaint);
				canvas.drawPath(path, mSelectorFramePaint);
				canvas.drawText(text, x - textWidth / 2, fixTextY1(y), mTextPaint);

				//绘制右边虚线
				canvas.drawLine(x + textWidth / 2 + w1 + w2, y, mWidth, y, mLastPricePaint);
			}
		}
	}
	...
	public void onChatClick(MotionEvent e) {
		this.clickY = e.getY();
		int lastIndex = mSelectedIndex;
		calculateSelectedX(e.getX());
		if (lastIndex != mSelectedIndex) {
	 ...
	 * @return
	 */
	public boolean isMainRect(float x, float y) {
		if (mMainRect == null) {
			return false;
		}
		boolean contains = mMainRect.contains((int) x, (int) y);
		return contains;
	}
	...
	public void drawMainMinuteLine(Canvas canvas, Paint paint, float startX, float startValue, float stopX, float stopValue) {
		...
		LinearGradient backGradient = new LinearGradient(startX, getMainY(startValue), stopX, displayHeight + mTopPadding + mBottomPadding,
				new int[]{Color.parseColor("#333b7bfd") ,Color.parseColor("#003b7bfd")}, null, Shader.TileMode.MIRROR);
		paint.setShader(backGradient);

		canvas.drawPath(path5, paint);

		//canvas.drawPath(path5, mFenshiBgPaint);
	}


	public void drawFenshiBg(Canvas canvas, Paint paint, float startX, float startValue, float stopX, float stopValue) {
		Drawable drawable = getResources().getDrawable(R.drawable.bg_chart_fenshi);
		drawable.setBounds((int) startX,
				(int)getMainY(startValue),
				(int) stopX,
				displayHeight + mTopPadding + mBottomPadding);
	...
	public String formatDateTime(String dateStr) {
		...
		try {
			...
			return getDateTimeFormatter().format(DateUtil.longTimeFormat.parse(dateStr));
	...
	public void setDefaultTextColor(int color) {
		mTextDefautePaint.setColor(color);
	}
	...
	public void setTextSize(float textSize) {
		...
		mTextDefautePaint.setTextSize(textSize);
	}
	...
	public void setMTextSize(float textSize) {
		...
		mLastPricePaint.setTextSize(textSize);
	}
	...
	public void setLineWidth(float lineWidth) {
		...
		mLastPricePaint.setStrokeWidth(lineWidth);
	}
	...
	public void setRedRise(boolean redRise) {
		isRedRise = redRise;
		mainDraw.updateRedRise(isRedRise);
	}
}
