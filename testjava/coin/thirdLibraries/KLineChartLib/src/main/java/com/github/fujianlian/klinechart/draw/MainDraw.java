...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private Paint fenshiBgPaint = new Paint(Paint.ANTI_ALIAS_FLAG);

    private Paint mSelectorTextPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mSelectorRedPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    private Paint mSelectorGreenPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    private IValueFormatter bigValueFormatter;
...
//        paint.setColor(ContextCompat.getColor(context, R.color.chart_line_background));
    ...
    public void drawTranslated(@Nullable ICandle lastPoint, @NonNull ICandle curPoint, float lastX, float curX, @NonNull Canvas canvas, @NonNull BaseKLineChartView view, int position) {
        if (isLine) {
            ...
            //view.drawFenshiBg(canvas, fenshiBgPaint, lastX, lastPoint.getClosePrice(), curX, curPoint.getClosePrice());
            if (status == Status.MA) {
                //画ma60
    ...
    public void drawText(@NonNull Canvas canvas, @NonNull BaseKLineChartView view, int position, float x, float y) {
        if (position >= view.getAdapter().getCount()) {
            return;
        }
        ...
        if (view.isLongPress || view.isClick) {
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        float height = padding * 11 + textHeight * 8;

        if (index >= view.getAdapter().getCount()) {
            return;
        }
        ICandle point = (ICandle) view.getItem(index);
        List<String> strings = new ArrayList<>();
        strings.add(String.format("%s*%s", getString(R.string.k_chart_open), view.formatValue(point.getOpenPrice())));
        strings.add(String.format("%s*%s", getString(R.string.k_chart_high), view.formatValue(point.getHighPrice())));
        strings.add(String.format("%s*%s", getString(R.string.k_chart_low), view.formatValue(point.getLowPrice())));
        strings.add(String.format("%s*%s", getString(R.string.k_chart_close), view.formatValue(point.getClosePrice())));

        if (point.getUpsAndDowns().equals("0.00%") || point.getUpsAndDowns().contains("-")) {
            strings.add(String.format("%s*%s", "涨跌额", view.formatValue(point.getChangeValue())));
            strings.add(String.format("%s*%s", "涨跌幅", point.getUpsAndDowns()));
        } else {
            strings.add(String.format("%s*+%s", "涨跌额", view.formatValue(point.getChangeValue())));
            strings.add(String.format("%s*+%s", "涨跌幅", point.getUpsAndDowns()));
        }
        strings.add(String.format("%s*%s", getString(R.string.entrust_amount), getBigValueFormatter().format(point.getVolume())));
        strings.add(String.format("%s*%s", getString(R.string.time), view.formatDateTime(view.getAdapter().getDate(index))));

//        for (String s : strings) {
//            width = Math.max(width, mSelectorTextPaint.measureText(s)) + 10;
//        }
//        width += padding * 2;

        width = ViewUtil.Dp2Px(mContext, 130);
        ...
        canvas.saveLayer(r, mSelectorBackgroundPaint);
        canvas.drawColor(mContext.getResources().getColor(R.color.chart_background));

        canvas.drawRoundRect(r, 0, 0, mSelectorBackgroundPaint);
        ...
        for (int i = 0; i < strings.size(); i++) {
            if (strings.get(i).contains("*")) {
                ...
                //draw key
                canvas.drawText(strs[0], left + padding + 10, y, mSelectorTextPaint);

                //draw value
                if (i == 4 || i == 5) {
                    //涨跌额和涨跌幅, 涨用红色的画笔，跌用绿色的画笔
                    if (strs[1].contains("+")) {
                        canvas.drawText(strs[1], left + width - padding - 10 - mSelectorTextPaint.measureText(strs[1]), y, mSelectorRedPaint);
                    } else {
                        canvas.drawText(strs[1], left + width - padding - 10 - mSelectorTextPaint.measureText(strs[1]), y, mSelectorGreenPaint);
                    }
                } else {
                    canvas.drawText(strs[1], left + width - padding - 10 - mSelectorTextPaint.measureText(strs[1]), y, mSelectorTextPaint);
                }
    ...
    public void setSelectorTextSize(float textSize) {
        ...
        mSelectorGreenPaint.setTextSize(textSize);
        mSelectorRedPaint.setTextSize(textSize);
    }
    ...
    public void updateRedRise(boolean isRedRise) {
        mRedPaint.setColor(ContextCompat.getColor(mContext, isRedRise ? R.color.chart_red : R.color.chart_green));
        mGreenPaint.setColor(ContextCompat.getColor(mContext, isRedRise ? R.color.chart_green : R.color.chart_red));
        mSelectorRedPaint.setColor(ContextCompat.getColor(mContext, isRedRise ? R.color.chart_red : R.color.chart_green));
        mSelectorGreenPaint.setColor(ContextCompat.getColor(mContext, isRedRise ? R.color.chart_green : R.color.chart_red));
    }

    public IValueFormatter getBigValueFormatter() {
        if (bigValueFormatter == null) {
            bigValueFormatter = new BigValueFormatter();
        }
