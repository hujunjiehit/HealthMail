...
public class VolumeDraw implements IChartDraw<IVolume> {
    ...
    private BigValueFormatter bigValueFormatter;
    private boolean isLine;

    public VolumeDraw(BaseKLineChartView view) {
        mContext = view.getContext();
        mRedPaint.setColor(ContextCompat.getColor(mContext, R.color.chart_red));
        mGreenPaint.setColor(ContextCompat.getColor(mContext, R.color.chart_green));
        pillarWidth = ViewUtil.Dp2Px(mContext, 6);
    }
    ...
    private void drawHistogram(Canvas canvas, IVolume curPoint, IVolume lastPoint, float curX,
            BaseKLineChartView view, int position) {
        if (isLine) {
            pillarWidth = ViewUtil.Dp2Px(mContext, 1);
        } else {
            pillarWidth = ViewUtil.Dp2Px(mContext, 6);
        }
