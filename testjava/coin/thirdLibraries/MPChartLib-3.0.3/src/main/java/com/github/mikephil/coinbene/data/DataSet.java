...
public abstract class DataSet<T extends Entry> extends BaseDataSet<T> {
    ...
    public void calcMinMaxY(float fromX, float toX) {
        ...
        try {
            ...
            for (int i = indexFrom; i <= indexTo; i++) {
                ...
                if (mValues.size() == 0 || i > mValues.size() - 1) {
                    return;
                }
                // only recalculate y
    ...
    public T getEntryForIndex(int index) {
        try {
            return (mValues == null || mValues.size() == 0) ? null : (index < mValues.size() ? mValues.get(index) : null);
        } catch (ArrayIndexOutOfBoundsException e) {
            return null;
        }
    }
    ...
    @Override
    public int getEntryIndex(float xValue, float closestToY, Rounding rounding) throws ArrayIndexOutOfBoundsException{
