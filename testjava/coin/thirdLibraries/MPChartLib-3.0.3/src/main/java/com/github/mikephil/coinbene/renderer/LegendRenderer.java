...
import com.github.mikephil.coinbene.components.LegendEntry;
...
import com.github.mikephil.coinbene.interfaces.datasets.IPieDataSet;
import com.github.mikephil.coinbene.utils.ColorTemplate;
import com.github.mikephil.coinbene.utils.FSize;
	...
	public void computeLegend(ChartData<?> data) {
		...
		if (!mLegend.isLegendCustom()) {
			...
			for (int i = 0; i < data.getDataSetCount(); i++) {
				...
				List<Integer> clrs = Collections.synchronizedList(dataSet.getColors());
					...
					synchronized (clrs) {
						for (int j = 0; j < clrs.size() && j < entryCount; j++) {
							...
							computedEntries.add(new LegendEntry(
									...
									dataSet.getFormLineWidth(),
									dataSet.getFormLineDashEffect(),
									clrs.get(j) != null ? clrs.get(j) : ColorTemplate.COLOR_NONE
							));
						}
					}
				}
