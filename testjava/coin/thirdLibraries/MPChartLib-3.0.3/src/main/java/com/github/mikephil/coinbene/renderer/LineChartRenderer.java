...
import com.github.mikephil.coinbene.interfaces.dataprovider.LineDataProvider;
...
public class LineChartRenderer extends LineRadarRenderer {
		...
		if (mDrawBitmap == null
			...
			if (width > 0 && height > 0) {
				...
				try {
					...
					if (mDrawBitmap.get() == null) {//可能为kong,导致下面崩溃
						return;
					}
					if (!mDrawBitmap.get().isMutable()) {
						return;
					}
	...
	protected void drawLinear(Canvas c, ILineDataSet dataSet) {
		...
		if (dataSet.getColors().size() > 1) {
			...
			if (mXBounds.min >= dataSet.getEntryCount()) {
				e1 = null;
			} else {
