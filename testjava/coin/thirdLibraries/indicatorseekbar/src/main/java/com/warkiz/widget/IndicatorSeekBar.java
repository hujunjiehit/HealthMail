...
public class IndicatorSeekBar extends View {
    ...
    private void drawTickTexts(Canvas canvas) {
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            if (i == 0) {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] + mTickTextsWidth[index] / 2.0f, mTickTextY - distanceY, mTextPaint);
            } else if (i == mTickTextsArr.length - 1) {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] - mTickTextsWidth[index] / 2.0f, mTickTextY - distanceY, mTextPaint);
            } else {
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i], mTickTextY - distanceY, mTextPaint);
            }
        }
    }
    ...
    public synchronized float getProgressFloat() {
        if (Double.isInfinite(mProgress) || Double.isNaN(mProgress)) {
            mProgress = 0f;
        }
    ...
    public void setDistanceY(int distanceY) {
        //tickerText距离track滑块距离减去distanceY
