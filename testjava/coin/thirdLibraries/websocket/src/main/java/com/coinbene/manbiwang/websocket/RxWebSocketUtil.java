...
public class RxWebSocketUtil {
            ...
            observable = Observable.create(new WebSocketOnSubscribe(url))
                    ...
                    .retry(new Predicate<Throwable>() {
                        ...
                        public boolean test(Throwable throwable) throws Exception {
                            if (showLog) {
                                Log.d(logTag, "test, throwable : " + throwable.getMessage());
                            }
                            return throwable instanceof IOException || throwable instanceof TimeoutException || throwable instanceof CustomException;
                        }
                    })
                    //共享
    ...
    public Observable<WebSocketInfo> getWebSocketInfo(String url) {
        return getWebSocketInfo(url, 20, TimeUnit.SECONDS);
    }
    ...
    private final class WebSocketOnSubscribe implements ObservableOnSubscribe<WebSocketInfo> {
        ...
        private void initWebSocket(final ObservableEmitter<WebSocketInfo> emitter) {
            webSocket = client.newWebSocket(getRequest(url), new WebSocketListener() {
                ...
                public void onMessage(WebSocket webSocket, String text) {
                    handleWsData(emitter, webSocket, text);
                }

                @Override
                public void onMessage(WebSocket webSocket, ByteString bytes) {
                    String uncompressText = GzipUtils.uncompress(bytes.toByteArray());
                    handleWsData(emitter, webSocket, uncompressText);
                }
                ...
                public void onClosing(WebSocket webSocket, int code, String reason) {
                    if (showLog) {
                        Log.d(logTag," --> onClosing, code = " + code + ", reason = " + reason);
                    }
                    webSocket.close(1000, null);
                    if (code != 3000) {
                        if (!emitter.isDisposed()) {
                            emitter.onError(new CustomException("custom:" + code));
                        }
                    }
                }
        ...
        private void handleWsData(ObservableEmitter<WebSocketInfo> emitter, WebSocket webSocket, String text) {
            if (showLog&& !TextUtils.isEmpty(text)) {
                if (text.contains("\"data\":")) {
                    ...
                    try {
                        ...
                        if (jsonObject.get("data") instanceof JsonArray) {
                            Log.d(logTag," --> onMessage = " + "topic: " + jsonObject.get("topic") + " data size:" + jsonObject.getAsJsonArray("data").size());
                        } else {
                            Log.d(logTag," --> onMessage = " + "topic: " + jsonObject.get("topic"));
                        }
                    } catch (Exception e) {
                        Log.d(logTag," --> onMessage = " + e.getMessage());
                    }
                } else {
            ...
            if (!emitter.isDisposed()) {
                ...
                webSocket.close(3001, "emitter isDisposed");
            }
        }
    }
}
